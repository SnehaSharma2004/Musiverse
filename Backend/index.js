    ...
    bcrypt.genSalt(10, (err, salt) => {
      bcrypt.hash(password, salt, async (err, hash) => {
        ...
        if (result.acknowledged) {
          ...
          res
            .status(200)
            .send({ msg: "Data inserted", insertid: result.insertedId ,token});
  ...
  try {
    ...
    if (!user) {
      ...
      bcrypt.compare(req.body.password, user.password, function (err, result) {
        if (result) {
          const token = jwt.sign({ userid: user._id,email }, TOKEN_SECRET_KEY, {
            expiresIn: "3 days",
          });
...
app.post("/getUser", async (req, res) => {
  ...
  jwt.verify(token, TOKEN_SECRET_KEY,async(err,decoded)=>{
        ...
        const user = await clientObj.db("music_player").collection("User").findOne({email:decoded.email});
        console.log({user});
