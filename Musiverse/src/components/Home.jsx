...
const Home1 = () => {
  ...
  const handleAddCommentClick = () => {
    setIsModalOpen(true); // Open the modal to add a new comment
  };
  ...
  const handleCommentSubmit = () => {
    ...
    const commentData = {
      ...
      text: newComment.trim(), // Add the trimmed comment text
  ...
  const handleCommentChange = (e) => {
    setNewComment(e.target.value); // Update the new comment input field
  };
  ...
  useEffect(() => {
    ...
    const getUserDetails = ()=>{
      ...
      .catch((error) => {
        console.error("error in getuset",error)
      });
    }
  ...
  return (
    <div
      className={`${
        lightMode ? "bg-slate-600 text-white" : "bg-gray-900 text-white"
      } min-h-screen`}
    >
      {/* Sidebar */}
      <div
      ...
      >
        ...
        {/* Comments Popup */}
        {comments.some((comment) => comment.trackId === currentTrack?._id) && (
          <div className="fixed top-12 right-4 bg-gray-900 p-4 rounded-lg w-96 shadow-lg z-40">
            <h3 className="text-lg font-semibold text-white mb-4">Comments</h3>
            <div className="comments-section space-y-2">
              {comments
                .filter((comment) => comment.trackId === currentTrack?._id)
                .map((comment) => (
                  <div
                    key={comment.id}
